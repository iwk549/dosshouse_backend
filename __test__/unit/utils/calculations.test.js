...
describe("calculations", () => {
  describe.only("createBinaryPlayoffTree", () => {
    it("should return null if playoff round 1 is not set", () => {
      const tree = createBinaryPlayoffTree(playoffMatches);
      expect(tree).toBeNull();
    });
    it("should return the tree, with the root at the final match", () => {
      const tree = createBinaryPlayoffTree(
        playoffMatches.map((m) => ({ ...m, teamsSet: m.round === 1 }))
      );
      const final = playoffMatches[playoffMatches.length - 1];
